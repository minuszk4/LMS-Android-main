...
exports.createMomoPayment = onRequest({ region: "asia-southeast1", cors: true }, async (req, res) => {
  if (req.method !== "POST") {
    return res.status(405).json({ resultCode: 99, message: "Method Not Allowed" });
  }
  ...
  try {
    const momoResult = await createMomoPayment(payload);
    if (!momoResult.ok) {
      logger.error("[createMomoPayment] MoMo create failed", momoResult.response);
      return res.status(400).json({
        resultCode: momoResult.response.resultCode || 1,
        message: momoResult.response.message || "Create payment failed",
        data: momoResult.response
    ...
    logger.error("[createMomoPayment] Internal error", error);
    return res.status(500).json({
      resultCode: 97,
      message: error.message || "Internal error"
    });
...
exports.momoIpnWebhook = onRequest({ region: "asia-southeast1", cors: true }, async (req, res) => {
  if (req.method !== "POST") {
    return res.status(405).json({ resultCode: 99, message: "Method Not Allowed" });
  }

  const payload = req.body && typeof req.body === "object" ? req.body : {};
  logger.info("[momoIpnWebhook] Incoming payload", payload);

  if (shouldVerifySignature()) {
    const verified = verifyMomoSignature(payload);
    if (!verified.ok) {
      logger.error("[momoIpnWebhook] Invalid signature", { reason: verified.reason });
      return res.status(400).json({ resultCode: 98, message: `Invalid signature: ${verified.reason}` });
    }
  }
  ...
  try {
    const saved = await upsertBankTransaction(payload);
    logger.info("[momoIpnWebhook] Saved bank transaction", saved);
    return res.status(200).json({ resultCode: 0, message: "received" });
  } catch (error) {
    logger.error("[momoIpnWebhook] Failed to persist transaction", error);
    return res.status(500).json({ resultCode: 97, message: "Internal error" });
  }
});
